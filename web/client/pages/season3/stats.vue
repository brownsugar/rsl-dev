...
export default {
  ...
  middleware ({ route: { query, matched }, redirect }) {
    if (matched.length === 1) {
      ...
      return redirect({
        path: '/season3/stats/' + defaultSlug,
        query
      })
    }
  },
  ...
  head () {
    const title = this.currentType.name + ' - S3 聯賽數據記錄'
  ...
  watch: {
    'currentType.name': {
      handler (val) {
        if (!val) {
          this.$router.replace('/season3/stats/' + this.children[0].slug)
        }
      },
      immediate: true
    }
